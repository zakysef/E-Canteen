...
<div class="flex h-full bg-pink-50"
     x-data="{
        expanded: localStorage.getItem('admin_sidebar') !== 'false',
        mobileOpen: false,
        toggle() { this.expanded = !this.expanded; localStorage.setItem('admin_sidebar', this.expanded) }
     }"
     x-cloak>
    ...
    <aside :class="[expanded ? 'w-64' : 'w-[70px]', mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0']"
        ...
        <nav class="flex-1 py-2 overflow-y-auto overflow-x-hidden space-y-0.5">
            ...
            $nav = [
                ...
                ['route' => 'admin.menu.index',       'icon' => 'ph-fork-knife',    'label' => 'Kelola Menu'],
